...
const CACHE_VERSION = '20251223-1125';
...
const urlsToCache = [
  // DO NOT cache HTML files - they must always be fetched fresh
  // '/',
  // '/index.html',
  // '/user-dashboard.html',
...
self.addEventListener('fetch', event => {
  ...
  // CRITICAL: HTML files (especially dashboards) MUST ALWAYS be fetched fresh - NEVER use cache
  // This ensures code changes are immediately visible and prevents stale content
  if (request.destination === 'document' || url.pathname.endsWith('.html') || url.pathname === '/' || url.pathname.includes('dashboard.html')) {
    event.respondWith(
      ...
      fetch(request, { 
        ...
        headers: {
          'Cache-Control': 'no-cache, no-store, must-revalidate',
          'Pragma': 'no-cache',
          'X-Cache-Bust': Date.now().toString() // Force fresh fetch
        }
      }).then(response => {
        // Delete any existing cache for this HTML file to prevent stale content
        caches.keys().then(cacheNames => {
          cacheNames.forEach(cacheName => {
            caches.open(cacheName).then(cache => {
              cache.delete(request).catch(() => {}); // Silently fail if not cached
            }).catch(() => {});
          });
        }).catch(() => {});
        // Return fresh response - NEVER cache HTML files
        return response;
      }).catch(() => {
        // Network failed - try to delete cache and return error (don't serve stale HTML)
        caches.keys().then(cacheNames => {
          cacheNames.forEach(cacheName => {
            caches.open(cacheName).then(cache => {
        ...
        // Don't serve cached HTML - return error instead
        return new Response('Page unavailable - please check your connection', { 
          status: 503,
          headers: {
            'Cache-Control': 'no-cache, no-store, must-revalidate',
            'Pragma': 'no-cache'
