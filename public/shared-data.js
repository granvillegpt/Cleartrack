...
var CleartrackDataManager = class CleartrackDataManager {
    ...
    async getPractitioner(practitionerId) {
        // Priority 1: Check Firestore if available
        if (window.firebaseDb && typeof window.firebaseDb.collection === 'function') {
            try {
                const doc = await window.firebaseDb.collection('users').doc(practitionerId).get();
                if (doc.exists) {
                    ...
                    try {
                        ...
                        const existingPractitioner = data.practitioners[practitionerId] || {};
                        data.practitioners[practitionerId] = { ...existingPractitioner, ...practitioner };
                        this.setData(data);
                    } catch (e) {
                        console.warn('Could not sync practitioner to localStorage:', e);
                    }
                    
                    return practitioner;
                }
            } catch (error) {
                console.warn('Error getting practitioner from Firestore, using localStorage:', error);
            }
        }
    ...
    async getPractitionerByCode(code) {
        // Priority 1: Use FirestoreDataManager if available
        if (window.firestoreData && typeof window.firestoreData.getPractitionerByCode === 'function') {
            try {
                const practitioner = await window.firestoreData.getPractitionerByCode(code);
                if (practitioner) return practitioner;
            } catch (error) {
                console.warn('Error getting practitioner from firestoreData, trying direct Firestore:', error);
            }
        }

        // Priority 2: Check Firestore directly if available
        if (window.firebaseDb && typeof window.firebaseDb.collection === 'function') {
            try {
                ...
                if (!snapshot.empty) {
                    ...
                    // Sync to localStorage for faster future lookups
                    try {
                        const data = this.getData();
                        if (!data.practitioners) data.practitioners = {};
                ...
                console.warn('Error querying Firestore directly, falling back to localStorage:', error);
            }
        }
    ...
    async connectUserToPractitioner(userId, practitionerId) {
        ...
        if (window.firestoreData && typeof window.firestoreData.connectUserToPractitioner === 'function') {
            try {
                const result = await window.firestoreData.connectUserToPractitioner(userId, practitionerId);
                // FirestoreDataManager already syncs to localStorage, so we're done
                return result;
            } catch (error) {
                console.warn('Error connecting via firestoreData, trying localStorage:', error);
            }
        }

        // Priority 2: Try Firestore directly if available
        if (window.firebaseDb && typeof window.firebaseDb.collection === 'function') {
            try {
                ...
                console.warn('Error connecting via Firestore directly, using localStorage only:', error);
            }
        }

        // Priority 3: Fallback to localStorage (always do this for offline support)
        try {
