...
exports.analyzeDocument = functions.https.onCall(async (data, context) => {
  try {
    ...
    // Validate input - accept either documentText or base64Image
    const { documentText, documentType, clientId, base64Image, mimeType, question } = data;
    
    if (!documentText && !base64Image) {
      throw new functions.https.HttpsError('invalid-argument', 'Either document text or image data is required');
    }

    // Initialize AI
    const genAI = getGenerativeAI();
    
    // Use vision-capable model for images/PDFs, text model for text-only
    // gemini-pro supports vision, but gemini-1.5-pro has better vision capabilities
    const isImageOrPDF = base64Image && (mimeType?.startsWith('image/') || mimeType === 'application/pdf');
    const modelName = isImageOrPDF ? 'gemini-1.5-pro' : 'gemini-pro';
    const model = genAI.getGenerativeModel({ model: modelName });

    // Create prompt for document analysis
    const basePrompt = `You are a tax document analysis assistant for South African tax practitioners. Analyze the following document and extract key information.
